...
parameters. The horseshoe prior (Carvalho et al., 2010) is a continuous
shrinkage prior for Bayesian variable selection with the hierarchical
structure: \$\$\beta_j \sim N(0, \lambda_j^2 \tau^2)\$\$ \$\$\lambda_j
\sim C^+(0, 1)\$\$ \$\$\tau \sim C^+(0, 1)\$\$ where \\C^+(0, 1)\\
denotes the half-Cauchy distribution, \\\lambda_j^2\\ are the local
shrinkage parameters (argument `theta`), and \\\tau^2\\ is the global
shrinkage parameter (argument `zeta`). The prior variance for
...
- coefs:
  ...
  **C++**: an `vec` vector object.

- theta:

  a \\p\\-vector with the local variance parameters \\\lambda_j^2\\,
  updated by reference. **C++**: an `vec` vector object.
...
- varpi:
  ...
  updated by reference. **C++**: a `double` scalar. **C++**: an `uvec`
  vector object (0-based indexing).
...
Vectorautoregressions. R package version 0.1.5,
